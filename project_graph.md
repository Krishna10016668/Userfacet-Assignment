# E-Library Management System — Project Graph & Architecture (v1.1.0)
...
├── README.md                     (7.8 KB)  — Project documentation
├── package.json                  (743 B)   — Node.js dependencies & scripts
├── package-lock.json             (64.2 KB) — Lockfile
├── project_graph.md              (16 KB)   — System architecture & ER diagram
├── walkthrough.md                (4.5 KB)  — Test verification walkthrough
├── start.bat                     (612 B)   — Dual-service launcher
│
├── src/                                     ── NODE.JS API SERVER (Port 3000) ──
│   ├── server.js                 (3.5 KB)  — Express entry point (v1.1.0)
│   │
│   ├── config/
│   │   └── index.js              (990 B)   — Centralized config loader
│   │
│   ├── database/
│   │   ├── connection.js         (944 B)   — SQLite singleton (WAL mode)
│   │   ├── schema.js             (9.2 KB)  — 14 tables + 17 indexes DDL
...
│   │   ├── book.routes.js        (4.8 KB)  — CRUD + Summary + Recs + Also-Borrowed
...
│   │   ├── book.service.js       (14.0 KB) — CRUD + search + AI proxy + Co-occurrence
│   │   ├── borrow.service.js     (11.2 KB) — Borrow engine + overdue + Audit logging
│   │   ├── reservation.service.js(7.3 KB)  — Queue management
│   │   ├── review.service.js     (4.9 KB)  — Review gates
│   │   ├── fine.service.js       (5.3 KB)  — Fine calculation + payment
...
│   │   ├── ai.service.js         (3.2 KB)  — Node→Python HTTP bridge
│   │   ├── readingProgress.service.js (4.5 KB) — 🌟 Reading velocity & ETA
...
│   │
│   ├── utils/
│   │   ├── constants.js          (1.8 KB)  — Enums (Roles, Statuses, Audit Actions)
...
│   ├── app.py                    (1.6 KB)  — Flask entry point
...
│   │
│   └── utils/
│       └── prompt_templates.py   (5.5 KB)  — GPT-4o-mini prompt templates
│
├── data/
│   ├── library.db                (220 KB)  — SQLite database (14 Tables, WAL Mode)
│   ├── library.db-shm            (32 KB)   — WAL shared memory
│   └── library.db-wal            (16 KB)   — Write-ahead log
│
└── tests/
    └── api.test.js               (9.5 KB)  — Full 20-Step Integration Test Suite
```
...
13. `reading_progress` — 🌟 Pages read, velocity, and completion ETA
14. `audit_log` — 🌟 Complete security and administrative audit trail
